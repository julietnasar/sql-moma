...
-- DEMONSTRATES: 1c (deathYear - birthYear), 1f (using as), 3i (IS NOT NULL), 6b
...
-- DEMONSTRATES 4b (group by with two columns), 6a
...
-- get all the art ordered from least to greatest
-- by the year they were finished and within that,
-- in reverse alphabetical order by their title
-- DEMONSTRATES: 6c
...
-- find the artworks completed after the avg date of all the artworks
-- DEMONSTRATES: 8ai
SELECT title, date
FROM artworks
WHERE date > (SELECT avg(date) FROM artworks);
 

-- find the artists born in 1990 or before
-- DEMONSTRATES: 7 (LIMIT)

SELECT birthYear, artistName
FROM artists
LIMIT 1990;


-- find the number of artworks of each medium of artists born after the average
-- birthYear of all artists sory ascending by the number of artworks
-- DEMONSTRATES: 8a
SELECT count(artworkID), medium, birthYear
FROM artworks JOIN artists ON artworks.artistID = artists.artistID
GROUP BY medium
HAVING birthYear < (SELECT avg(birthYear) FROM artists)
ORDER BY count(artworkID) ASC;

-- find the art that can be hung on a wall, that is a classification of Photograph, Painting, Print or Drawing
-- DEMONSTRATES: 8aii

SELECT title, classification
FROM artworks 
WHERE classification IN (SELECT classification FROM artworks WHERE classification IN ('Photograph','Painting','Print','Drawing'));

-- find all the nationalities of artists available that we DO NOT have of artists in our gallery
-- DEMONSTRATES: 8b
SELECT nationality FROM nationalities
EXCEPT
SELECT nationality FROM artists_nationalities;

-- find all the artists who have art classified as 'Design'
-- in this dataset
-- DEMONSTRATES: 8b

SELECT artistName
FROM artworks JOIN artists ON artworks.artistID = artists.artistID 
WHERE classification = 'Design'
INTERSECT
SELECT artistName 
FROM artists;
